...
def display_generate_page():
    ...
    with tab1:
        ...
        with st.expander("📚 Prompt Library - Quick Access", expanded=False):
            col1, col2 = st.columns([1, 3])
            with col1:
                prompt_category = st.selectbox(
                    "Category",
                    list(PROMPT_LIBRARY.keys()),
                    key="txt2img_prompt_category"
                )
            
            with col2:
                all_prompts = PROMPT_LIBRARY[prompt_category] + st.session_state.custom_prompts.get(prompt_category, [])
                selected_prompt = st.selectbox(
                    f"Select from {len(all_prompts)} prompts",
                    [""] + all_prompts,
                    key="txt2img_selected_prompt",
                    format_func=lambda x: "Choose a prompt..." if x == "" else x[:80] + "..." if len(x) > 80 else x
                )
...
def display_generate_page():
    ...
    with tab2:
        ...
        with st.expander("📚 Prompt Library - Image Edits", expanded=False):
            col1, col2 = st.columns([1, 3])
            with col1:
                prompt_category_qwen = st.selectbox(
                    "Category",
                    list(PROMPT_LIBRARY.keys()),
                    key="qwen_prompt_category",
                    index=2  # Default to "Image Edits"
                )
            
            with col2:
                all_prompts_qwen = PROMPT_LIBRARY[prompt_category_qwen] + st.session_state.custom_prompts.get(prompt_category_qwen, [])
                selected_prompt_qwen = st.selectbox(
                    f"Select from {len(all_prompts_qwen)} prompts",
                    [""] + all_prompts_qwen,
                    key="qwen_selected_prompt",
                    format_func=lambda x: "Choose a prompt..." if x == "" else x[:80] + "..." if len(x) > 80 else x
                )
        ...
        with st.form("qwen_image_edit_form"):
            default_qwen_prompt = selected_prompt_qwen if selected_prompt_qwen else "Make the image more vibrant and colorful"
            prompt = st.text_area("Edit Prompt", default_qwen_prompt, key="qwen_prompt")
...
def display_generate_page():
    ...
    with tab3:
        ...
        with st.expander("📚 Prompt Library - E-commerce & Professional", expanded=False):
            col1, col2 = st.columns([1, 3])
            with col1:
                prompt_category_seedream = st.selectbox(
                    "Category",
                    list(PROMPT_LIBRARY.keys()),
                    key="seedream_prompt_category",
                    index=0  # Default to "E-commerce Mockups"
                )
            
            with col2:
                all_prompts_seedream = PROMPT_LIBRARY[prompt_category_seedream] + st.session_state.custom_prompts.get(prompt_category_seedream, [])
                selected_prompt_seedream = st.selectbox(
                    f"Select from {len(all_prompts_seedream)} prompts",
                    [""] + all_prompts_seedream,
                    key="seedream_selected_prompt",
                    format_func=lambda x: "Choose a prompt..." if x == "" else x[:80] + "..." if len(x) > 80 else x
                )
        ...
        with st.form("seedream_image_edit_form"):
            default_seedream_prompt = selected_prompt_seedream if selected_prompt_seedream else "Create a tshirt mock up with this logo"
            prompt = st.text_area("Edit Prompt", default_seedream_prompt, key="seedream_prompt")
